.data wel: .asciiz "Welcome to Word Soup!\n"game:  .asciiz "I am thinking of a word.  The word is  "game2: .asciiz "The word is "sco: .asciiz "  Round score is "guess: .asciiz "\nGuess any letter a-z\nenter a '.' to forfeit\nenter a '?' for a hint\nenter a'!' if you think you can solve the word on one guess\nGuess a letter?\n"yes: .asciiz "\nYes! "no: .asciiz "\nNo! "end1: .asciiz "\nYour final game tally is "end2: .asciiz ". Goodbye!"win: .asciiz "You won the round! Your final guess was: \n"
...
yesl:addi $s3,$s3,1 			#counter for correct loop
	j pregoodend
repeated:lw $a0, ($t7)			#get print yes word
	li $v0,4
	syscall
	j backpremain
...
fullgoodend:lb $s4,($s6)			#test byte from original word, to game word
	...
	j fullgoodend
prerepeated:beq $s5,0x2a,nexttest
	j repeated
