...
MOVE_FORWARD_FORGIVEN_DURATION  = 13

TURN_IN_PLACE                   = 20
TURN_IN_PLACE_FORDURATION       = 21
...
absMaxTrackBotVelocity = 0.18 # Allow tracking bot to go faster than the master bot
absMinTrackBotVelocity = 0.03
...
numOdometryMessagesSent = 0
odemetryMessageFrequency = 1
globalTargetPositionMessage = None

includeExtendedMove = False
...
#####################################################################################################
class PIDController:
    def __init__( self, _pc, _ic, _dc, _minOutput, _maxOutput, clamp ):
        self.pc = _pc
        self.ic = _ic
        self.dc = _dc
...
def getErrorBetweenBots( myPositionMessage, targetPositionMessage ):
    ...
    print "MyPostion=," + str( myPositionMessage.x ) + ", HisPosition=," + str( targetPositionMessage.x )
...
def odometryCallback( odometry ):
    ...
    global myBotStatus, forwardMoveConfiguration, turningConfiguration, numOdometryMessagesSent
...
#print "Move type: " + str(moveType)
    ...
    elif ( moveType == MOVE_FORWARD_FORGIVEN_DURATION ):
        print "HELLOW WORLD FORWARD"
        if ( globalStopMove ):
            print "GLOBAL STOP FORWARD MOVE"
            stopMoving( odometry, forwardMoveConfiguration.getPID() )

    elif ( moveType == TURN_IN_PLACE_FORDURATION ):
        print "HELLOW WORLD"
        if ( globalStopMove ):
            print "GLOBAL STOP MOVE"
            stopMoving( odometry, None )
    ...
    if ( useOdometryForLocalization ):
        ...
        if ( numOdometryMessagesSent % odemetryMessageFrequency == 0 or moveType == MOVE_NONE ):
            locationPublisher.publish( botLocations )
        numOdometryMessagesSent = numOdometryMessagesSent + 1
    ...
    if ( myBotStatus != None ):
        ...
        print "BotStatusMessageIsNULL - BAD!"
...
def otherBotStopped():
    ...
    print "Other Bot Status = " + str( otherBotStatus.getMoveType() )
    
    return ( otherBotStatus.getMoveType() == MOVE_NONE ) or ( lastOdometryMessage.pose.pose.position.x - globalTargetPositionMessage.x > 0.45 )
...
def botPositionEventCallback( botPositionMessage ):
    ...
    global endGoal, boxPosition, myBotStatus, globalTargetPositionMessage
    ...
    if ( not useOdometryForLocalization ):
        ...
        targetPositionMessage = botPositionMessage.robots[0]
    ...
    globalTargetPositionMessage = targetPositionMessage
...
def moveForward( xTarget, yTarget, angularVelocity, xTolerance, yTolerance, minVelocity, maxVelocity ):
    ...
    pid = PIDController( 0.35, 0.001, 0.1, minVelocity, maxVelocity, True )
...
def moveTrackingOtherBot( minVelocity, maxVelocity ):
    ...
    # _pc, _ic, _dc, _minOutput, _maxOutput ):
    pid = PIDController( 3.0, 0.0, 0.0, minVelocity, maxVelocity, True )
...
#####################################################################################################
#####################################################################################################
def moveForDuration( velocity, duration ):
    global moveType, moveCommand, trackBotConfiguration, globalStopMove
    ...
    # _pc, _ic, _dc, _minOutput, _maxOutput ):
    pid = PIDController( 3.0, 0.0, 0.0, velocity, velocity, True )
    pid.setTarget( 0.0 ) # TODO:  Add to the constructor, but leave set in case
    forwardMoveConfiguration = ForwardMoveConfiguration( 2, 0.3, 2.0, 0.3, pid )
    print pid.getCSVHeader()
    
    # Wait for the move to complete then pause for 0.5 seconds
    globalStopMove = False
    moveType = MOVE_FORWARD_FORGIVEN_DURATION
...
def moveTrackingOtherBotSimulation():
    ...
    # _pc, _ic, _dc, _minOutput, _maxOutput ):
    pid = PIDController( 0.5, 0.0, 0.0, -0.6, 0.6, False )
    pid.setTarget( 0.0 ) # TODO:  Add to the constructor, but leave set in case
    print pid.getCSVHeader()
    trackBotConfiguration = TrackBotConfiguration( botToTrackID, pid )
...
def turnBot( targetInDegress, toleranceInDegress, velocity ):
    ...
    pid = PIDController( 10.0, 0.000, 0.0, velocity, 0.5, True )
...
#####################################################################################################
#####################################################################################################
def turnForDuration( velocity, duration ):
    global moveType, moveCommand, turningConfiguration, globalStopMove
    
    pid = PIDController( 10.0, 0.000, 0.0, velocity, 0.5, True )
    
    turningConfiguration = TurningBotConfiguration( 180, 5, pid )
    ...
    globalStopMove = False
    moveType = TURN_IN_PLACE_FORDURATION
    print "SLEEP FOR TURN"
    rospy.sleep( duration )
    print "DONE SLEEP FOR TURN"
    globalStopMove = True
    rospy.sleep( 5.0 )
...
def executeMasterBot( ):
    ...
    moveForward( 5.0, 0.0, 0.0, 0.05, 1.0, 0.05, 0.15 ) # Allow the y to be off a good bit
    
    if ( includeExtendedMove ):
        print "Master Bot : Turning the box 90 degrees"
        #moveForward( xTarget, yTarget, angularVelocity, xTolerance, yTolerance, minVelocity, maxVelocity )
        # TODO: Balance the X Target and the angular velocity
        # TODO: Figure out the Y target
        moveForward( 0.3, 0.3, 0.3, 0.01, 0.3, 0.05, 0.15 ) # Allow the y to be off a good bit
        ...
        print "Master Bot : Moving forward x meters"
        # TODO: Figure out the Y target
        # TODO: Figure out the X target
        moveForward( 5.5, 1.0, 0.0, 0.25, 0.25, 0.05, 0.15 ) # Allow the y to be off a good bit
...
def executeTrackingBot():
	...
	#moveType = MOVE_FORWARD_SPECIFIC_DISTANCE
	#moveForward( .10, 0.00, 0.00, 0.01, 0.01, 0.2, 0.25 )
    print "Tracking Bot : Making contact with object"
	#waitForOtherBotToStop()
    makeContactWithObject( 0.05 )
    ...
    moveTrackingOtherBot( -0.005, 0.005 ) # TODO: Balance the clamps
    print "Tracking Bot : In extended move"

    if ( includeExtendedMove and False ):
        rospy.sleep( 10.0 )

        # wait for the master bot to spin to 90 degrees and stop
        print "Tracking Bot : Wait for other bot to start turning"
        waitForOtherBotToStart()
        print "Tracking Bot : Wait for other bot to turn the box 90 degrees"
        waitForOtherBotToStop()
        print "Tracking Bot : Waiting for master bot to make contact"
        waitForOtherBotToMakeContact()

        # Move forward a little bit to account for the box moving away during the turn
        #moveForward( xTarget, yTarget, angularVelocity, tolerance, minVelocity, maxVelocity )
        #moveForward( 0.05, 0.0, 0.0, 0.01, 1.0, 0.05, 0.15 ) # Allow the y to be off a good bit

        # Turn towards the box and make contact with it
        turnForDuration( 0.5, 14.0 )
        moveForDuration( 0.15, 2.0 ) # Allow the y to be off a good bit
        turnForDuration( 0.5, 5.0 )
        makeContactWithObject( 0.05 )
        ...
        print "Tracking Bot : Moving forward keeping up with master"
        moveTrackingOtherBot( -0.05, 0.005 )
...
def initializeCommands( myBotName, otherBotName, masterBot ):
    ...
    if ( useOdometryForLocalization ):
        locationPublisher = rospy.Publisher( "/" + otherBotName + "/locations", LocationList, tcp_nodelay=False, queue_size=2 ) # Use odometry for localization
    
    #, latch=True, queue_size=10
    print "Publish Queue Name => " + "/" + myBotName + "/botStatus"
    botStatusPublisher = rospy.Publisher( myBotName + "/botStatus", BotStatus, queue_size=2 ) # Publish Bot Status
    ...
    #       we did not mean to.
    rospy.Subscriber( myBotName + "_odom", Odometry, odometryCallback, queue_size=2000 )
    rospy.Subscriber( "/mobile_base/events/bumper", BumperEvent, bumperEventCallback, queue_size=1000 )
    rospy.Subscriber( "/" + myBotName + "/locations", LocationList, botPositionEventCallback, queue_size=2 )
    ...
    rospy.Subscriber( "/" + otherBotName + "/botStatus", BotStatus, botStatusCallback, queue_size=1000 )
...
if __name__ == "__main__":
    try:
        ...
        # Fifth parameter is used to enable the extended move functionality (turn and move)
        includeExtendedMove = int(sys.argv[5]) == 1
        ...
        #turnBot( 270, 0.5, 0.4 )
        #turnBot( 360, 0.5, 0.4 )
        #turnForDuration( 0.5, 16.0 )
        #rospy.sleep(10.0)
        #turnForDuration( 0.5, 4.0 )
        if ( masterBot == 1 ):
            executeMasterBot()
        else:
            executeTrackingBot()
