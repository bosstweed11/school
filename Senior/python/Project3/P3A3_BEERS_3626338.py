...
def restart_game( wordToGuess, hint, lettersLeft, guessedWord, guessedLetters, strikes, words):

    # Get a random word
    element = words.popitem()

    # Extract hint and word from the element
    wordToGuess = element[0]
    hint = element[1]
    ...
    return wordToGuess, guessedWord, strikes, guessedLetters, lettersLeft, hint
...
def activity_one( words ):
    ...
    lettersLeft = 0
    hint = []
    wordToGuess, guessedWord, strikes, guessedLetters, lettersLeft, hint = restart_game( wordToGuess, hint, lettersLeft, guessedWord, guessedLetters, strikes, words)
    ...
    while( playing == 1):
        ...
        letter = input("Which letter would you like to guess?\n(Enter * for a one shot guess at the word)")[0].lower()

        # If the 
        if ( letter == "*" ):
            wordGuess = input("Guess the entire word: ")

            if ( wordGuess == "".join(wordToGuess) ):
                lettersLeft = 0
            else:
                strikes = 6
        else:
...
def activity_two():
    ...
    filename = "wordsWithHints.txt"
    dictionary = {}
    file = open(filename,'r');
    i = 0
    listOfLines = list(file.readlines())
    for i in range(len(listOfLines)):
        if ( i % 2 == 0 ):
            #print("Element: %s :: %s\n" % ( listOfLines[i].rstrip(), listOfLines[i+1].rstrip()) )
            dictionary[listOfLines[i].rstrip()] = listOfLines[i+1].rstrip()
            #print("Element: %s :: %s\n" % ( listOfLines[i].rstrip(), dictionary[listOfLines[i].rstrip()] ) )
        i +=1
...
if __name__ == "__main__":
    ...
    wordsWithHints = activity_two()

    print("test if dictionary coes back\n")
    print("Element Fun: %s\n" % wordsWithHints["fun"])
    
    # Function for Activity 1
    activity_one( wordsWithHints )        
