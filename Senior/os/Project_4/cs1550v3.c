				...
				for ( i = 0; i < temp_dir->nFiles; i++ )
				{
					if ( strcmp( temp_dir->files[i].fname, filename ) == 0 && strcmp( temp_dir->files[i].fext, extension) == 0 )
					{
						if ( size > 0 && offset <= size )
						{
							...
							if ( disk_fp)
								printf("opened the disk: %x\n", disk_fp);
							else
								printf("failed opening the disk\n");
							...
							if ( size > BLOCK_SIZE )
							{
								...
								while ( size > BLOCK_SIZE )
								{
									...
									if ( fwrite( block, sizeof( cs1550_disk_block), 1, disk_fp) != 1 )
								...
								// Go to the start block of the file
								if ( fseek( disk_fp, (blockToWrite + count ) * sizeof (cs1550_disk_block), SEEK_SET) != 0 )
									printf("Seek to last block failed\n");
								...
								if ( fwrite( free_space, sizeof( cs1550_disk_block), 1, disk_fp) != 1 )
									printf("Write freespace failed\n");
								
								printf("wrote freespace\n");
							...
							else
							{
								...
								// Write the data to the .disk
								int writeResult = fwrite( block, sizeof( cs1550_disk_block), 1, disk_fp);
								printf("fwrite wrote %d bytes\n", writeResult);
								if ( writeResult != sizeof(cs1550_disk_block) )
									printf("write block failed\n");
