...
void my_keyboard( unsigned char key, int x, int y ) 
{
  ...
  GLfloat vol_change = ((float)(rand() % 4) + .5)/4. ;
  BASS_ChannelSetAttribute(sounds[num], BASS_ATTRIB_VOL, vol_change);
  printf("vol_change is %f\n", vol_change);
  ...
  case '1': {
  	 make_explosion(&explosions[0], 3.5, new_density, 800, cur_cube_size, new_after_counter, cur_shape, test_sound, (&nozzles[0])->x, (&nozzles[0])->y + 1.2);
...
void my_display()
{
 ...
 if (shift_camera) {
  ...
  if (light_screen) {
   ...
   printf("light_screen is %d", light_screen);
   light_screen++;
