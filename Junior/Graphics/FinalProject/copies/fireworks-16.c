...
GLfloat cur_radius = 70;
GLfloat cur_density = 1;
int cur_quantity = 200;
GLfloat cur_cube_size = .02;
int cur_after_counter = 10;
int cur_shape = 1;
...
void my_keyboard( unsigned char key, int x, int y ) 
{
  ...
  case '1': {
    make_explosion(&explosions[0], 70, 1, 5000, .02, 48, 1, test_sound, (&nozzles[0])->x, (&nozzles[0])->y + 1);
    ...
    for (i = 0; i < (&explosions[j])->quantity; i++) {
      ...
      if (cur_explosion->counter < cur_explosion->radius) {
        ...
        if (cur_explosion->counter > cur_explosion->radius && cur_explosion->counter < cur_explosion->radius + cur_explosion->after_counter && cur_explosion -> radius > 0) {
          real_translation(&(cur->shape), 0, -.01, 0);
