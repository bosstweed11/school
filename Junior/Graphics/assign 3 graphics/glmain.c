...
void real_rot_arbitrary_axis_point(GLfloat deg, GLfloat ax, GLfloat ay, GLfloat az, GLfloat cx, GLfloat cy, GLfloat cz ) {
	...
	//origin
		v_start[0] = local_axes[0][0];
		v_start[1] = local_axes[0][1];
		v_start[2] = local_axes[0][2];
		v_start[3] = local_axes[0][3];
		...
		//"X" axis
		v_start[0] = local_axes[1][0];
		v_start[1] = local_axes[1][1];
		v_start[2] = local_axes[1][2];
		v_start[3] = local_axes[1][3];
		...
		set_T(-cx,-cy,-cz);
		//"Y" axis
		v_start[0] = local_axes[2][0];
		v_start[1] = local_axes[2][1];
		v_start[2] = local_axes[2][2];
		v_start[3] = local_axes[2][3];
		...
		set_T(-cx,-cy,-cz);
		//"Z" axis
		v_start[0] = local_axes[3][0];
		v_start[1] = local_axes[3][1];
		v_start[2] = local_axes[3][2];
		v_start[3] = local_axes[3][3];
	...
	//same transformation for all shapes
if(crt_shape == SPHERE){
	for(i=0;i<crt_vs;i++){
		for(j=0;j<crt_rs;j++){
		v_start[0] = vertices_sph[i][j][0];
		v_start[1] = vertices_sph[i][j][1];
		v_start[2] = vertices_sph[i][j][2];
		v_start[3] = vertices_sph[i][j][3];
		matrix_mult(T,v_start);
		matrix_mult(RR,result);
...
else if(crt_shape == CYLINDER){

	for(i=0;i<crt_vs;i++){
		for(j=0;j<crt_rs;j++){
		v_start[0] = vertices_cyl[i][j][0];
		v_start[1] = vertices_cyl[i][j][1];
		v_start[2] = vertices_cyl[i][j][2];
		v_start[3] = vertices_cyl[i][j][3];
		matrix_mult(T,v_start);
		matrix_mult(RR,result);
	...
	for(i=0;i<crt_vs;i++){
		for(j=0;j<crt_rs;j++){
		v_start[0] = vertices_con[i][j][0];
		v_start[1] = vertices_con[i][j][1];
		v_start[2] = vertices_con[i][j][2];
		v_start[3] = vertices_con[i][j][3];
		matrix_mult(T,v_start);
		matrix_mult(RR,result);
	...
	for(i=0;i<crt_vs;i++){
		for(j=0;j<crt_rs;j++){
		v_start[0] = vertices_tor[i][j][0];
		v_start[1] = vertices_tor[i][j][1];
		v_start[2] = vertices_tor[i][j][2];
		v_start[3] = vertices_tor[i][j][3];
		matrix_mult(T,v_start);
		matrix_mult(RR,result);
...
void real_rotation(GLfloat deg, GLfloat x, GLfloat y, GLfloat z) {
	...
	//use rotation on the current object
	if(crt_shape == SPHERE){
		for(i=0;i<crt_vs;i++){
			for(j=0;j<crt_rs;j++){
			v_start[0] = vertices_sph[i][j][0];
...
void make_sphere(double ray, int rs, int vs)
{
  ...
  if(ray!=0){
  	...
  	v_start[0] = vertices_sph[0][0][0];
  	v_start[1] = vertices_sph[0][0][1];
  	v_start[2] = vertices_sph[0][0][2];
  	v_start[3] = vertices_sph[0][0][3];
