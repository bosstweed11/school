...
void real_rot_arbitrary_axis_point(GLfloat deg, GLfloat ax, GLfloat ay, GLfloat az, GLfloat cx, GLfloat cy, GLfloat cz ) {
	...
	//origin
		...
		for(i=0;i<crt_vs;i++){
		for(j=0;j<crt_rs;j++){
		(*gen).v_start[0] = (*gen).vertices[i][j][0];
		(*gen).v_start[1] = (*gen).vertices[i][j][1];
		(*gen).v_start[2] = (*gen).vertices[i][j][2];
		(*gen).v_start[3] = (*gen).vertices[i][j][3];
		matrix_mult(T,(*gen).v_start);
		matrix_mult(RR,result);
		...
		set_T(cx,cy,cz);
		matrix_mult(T,result);
		(*gen).vertices[i][j][0] =result[0];
		(*gen).vertices[i][j][1] =result[1];
		(*gen).vertices[i][j][2] =result[2];
		(*gen).vertices[i][j][3] =result[3];
