        ...
        ////FOR INTRO TEXT
	  	...
	  	if (curr_fuse->start_y==-1){
           ...
           cur_shape = global_shape;
            ...
            make_fuse(&fuses[cur_fuse = (cur_fuse + 1 ) % MAX_FIREWORKS], 15, colors[RED], .005, .005, 0, .004, curr_fuse -> start_x, test_this_shit_out);
            make_fuse(&fuses[cur_fuse = (cur_fuse + 1 ) % MAX_FIREWORKS], 15, colors[RED], -.005, .005, 0, .004, curr_fuse -> start_x, test_this_shit_out);
            make_fuse(&fuses[cur_fuse = (cur_fuse + 1 ) % MAX_FIREWORKS], 15, colors[RED], -.005, -.005, 0, .004, curr_fuse -> start_x, test_this_shit_out);
            make_fuse(&fuses[cur_fuse = (cur_fuse + 1 ) % MAX_FIREWORKS], 15, colors[RED], .005, -.005, 0, .004, curr_fuse -> start_x, test_this_shit_out);
          }
