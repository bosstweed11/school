...
public class LinearProbingHashST<Key, Value> implements STInterface<Key, Value>
{
    ...
    private boolean resize; 	 //if resizing is allowed
    ...
    //resize option added
    public LinearProbingHashST(int capacity, boolean resizeOption) 
    {
        M = capacity;
    ...
    private void resize(int capacity) 
    {
    	if ( resize )
    	{
			...
			LinearProbingHashST<Key, Value> temp = new LinearProbingHashST<Key, Value>(capacity, true);
			for (int i = 0; i < M; i++) 
			{
				if (keys[i] != null) 
				{
